...
import { getApiUrl, formatCurrency, formatNumber, formatNumberWithSuffix, formatCostBasis } from "@/lib/utils"
...
export default function EntitiesCompaniesTable({ activeTab, setActiveTab, loading, entityTableData, setEntityTableData, entityTableLoading, isMobile }: { activeTab: string, setActiveTab: (tab: string) => void, loading: boolean, entityTableData: any[], setEntityTableData: (data: any[]) => void, entityTableLoading: boolean, isMobile: boolean }) {
  ...
  return (
    <Card data-table="companies" className="group relative overflow-hidden border-0 bg-gradient-to-br from-card/80 to-card/40 backdrop-blur-sm transition-all duration-700 hover:shadow-2xl hover:shadow-orange-500/20 dark:hover:shadow-orange-400/10">
...
const MobileEntitiesTable = ({
  ...
  const MobileTableRowShimmer = () => (
    <TableRow className="border-b border-border/50">
      ...
      <TableCell className="py-1 px-1 text-right w-[10%]">
        <Shimmer className="h-3 w-6 ml-auto" />
      </TableCell>
      <TableCell className="py-1 px-1 text-right w-[8%]">
    ...
    <Table className="w-full table-fixed">
      <TableHeader className="sticky top-0 z-10 bg-gradient-to-r from-orange-50/90 to-orange-100/70 dark:from-orange-950/90">
        <TableRow className="hover:bg-transparent">
          <TableHead className="py-1 px-1 font-medium text-[9px] w-[4%] sticky left-0 bg-gradient-to-r from-orange-50/95 to-orange-100/75 dark:from-orange-950/95">
            {renderSortableHeader('rank', '#', 'Entity ranking')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] w-[4%]">

          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] w-[20%]">
            {renderSortableHeader('name', 'Name', 'Company or entity name')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] text-left w-[12%]">
            {renderSortableHeader('bitcoinHoldings', 'Bitcoin', 'Total Bitcoin holdings')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] text-left w-[14%]">
            {renderSortableHeader('usdValue', 'USD Value', 'Current value of Bitcoin holdings')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] text-left w-[16%]">
            {renderSortableHeader('marketCap', 'M.Cap', 'Total market capitalization')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] text-left w-[10%]">
            {renderSortableHeader('ngu', 'NgU', 'Number go up')}
          </TableHead>
          <TableHead className="py-1 px-1 font-medium text-[9px] text-left w-[10%]">
        ...
        {loading ? (
          ...
          finalSortedEntities.map((entity: Entity, index: number) => (
            ...
            >
              ...
              <TableCell className="py-1 px-1 w-[8%] border-r border-border/20 font-mono tracking-tight text-[9px]">
                <span className="text-[9px] text-foreground font-mono tracking-tight">{entity.countryFlag || '🏳️'}</span>
              </TableCell>
              <TableCell className="py-1 px-1 w-[20%] border-r border-border/20 overflow-hidden font-mono tracking-tight text-xs">
                <CustomLink href={`/${entity.slug}`} className="group/link flex items-center hover:text-orange-600 dark:hover:text-orange-400 transition-colors duration-300">
                  {entity.slug === "matador-technologies-inc" && (
                    <Star className="text-orange-500 dark:text-orange-400 h-2 w-2 flex-shrink-0 mr-0.5" fill="currentColor" />
                  )}
              ...
              <TableCell className="py-1 px-1 text-left w-[9%] font-mono tracking-tight text-[9px]">
              ...
              <TableCell className="py-1 px-1 text-left w-[15%] font-mono tracking-tight text-[9px]">
                <span className="text-[9px] text-foreground font-mono tracking-tight">
                  {entity.usdValue ? `$${Math.round(Number(entity.usdValue) / 1000000).toLocaleString()}M` : '—'}
                </span>
              </TableCell>

              <TableCell className="py-1 px-1 text-left w-[10%] font-mono tracking-tight text-[9px]">
                {entity.marketCap ? (
                  <span className="text-[9px] text-foreground font-mono tracking-tight">
                    ${Math.round(Number(entity.marketCap) / 1000000).toLocaleString()}M
                  </span>
                ) : (
                  <span className="text-muted-foreground text-[9px] font-mono tracking-tight text-left">—</span>
                )}
              </TableCell>
              ...
              <TableCell className="py-1 px-1 text-left w-[9%] font-mono tracking-tight text-[9px]">
                <span className="text-[9px] text-foreground font-mono tracking-tight">
                  {entity.supplyPercentage ? Number(entity.supplyPercentage).toFixed(3) : '0.000'}%
                </span>
              </TableCell>
            </TableRow>
          ))
...
const DesktopEntitiesTable = ({
  ...
  return (
    <Table className="w-full table-fixed">
      <TableHeader className="sticky top-0 z-10 bg-gradient-to-r from-orange-50/90 to-orange-100/70 dark:from-orange-950/90">
        <TableRow className="hover:bg-transparent">
          <TableHead className="py-3 px-2 font-medium text-sm w-[6%] sticky left-0 text-left bg-gradient-to-r from-orange-50/95 to-orange-100/75 dark:from-orange-950/95 font-mono tracking-tight text-xs">
            {renderSortableHeader('rank', '#', 'Entity ranking')}
          </TableHead>
          ...
          <TableHead className="py-3 px-2 font-medium text-sm text-right w-[7%] font-mono tracking-tight text-xs">
            {renderSortableHeader('enterpriseValue', 'Ent. Value', 'Enterprise value')}
          </TableHead>
          <TableHead className="py-3 px-2 font-medium text-sm text-right w-[7%] font-mono tracking-tight text-xs">
            {renderSortableHeader('btcPerShare', 'BTC/Share', 'Bitcoin per share')}
          </TableHead>
          <TableHead className="py-3 px-2 font-medium text-sm text-right w-[7%] font-mono tracking-tight text-xs">
            {renderSortableHeader('costBasis', 'Cost Basis', 'Average cost basis per Bitcoin')}
          </TableHead>
          <TableHead className="py-3 px-2 font-medium text-sm text-right w-[7%] font-mono tracking-tight text-xs">
            {renderSortableHeader('ngu', 'NgU', 'Number go up')}
          </TableHead>
          <TableHead className="py-3 px-2 font-medium text-sm text-right w-[7%] font-mono tracking-tight text-xs">
            {renderSortableHeader('mNav', 'mNAV', 'Modified net asset value (mNAV)')}
          </TableHead>
          <TableHead className="py-3 px-2 font-medium text-sm text-left w-[8%] font-mono tracking-tight text-xs">
            {renderSortableHeader('supplyPercentage', '/21M', 'Percentage of total Bitcoin supply (21M)')}
          </TableHead>
        </TableRow>
      </TableHeader>
      <TableBody className="divide-y divide-border/30">
        {loading ? (
          ...
          finalSortedEntities.map((entity: Entity, index: number) => (
            ...
            >
              <TableCell className="py-3 px-2 w-[6%] border-r border-border/20">
              ...
              <TableCell className="py-3 px-2 w-[6%] border-r border-border/20 font-mono tracking-tight text-xs">
                <span className="text-xs text-foreground font-mono tracking-tight">{entity.countryFlag || '🏳️'}</span>
              </TableCell>
              <TableCell className="py-3 px-2 w-[20%] border-r border-border/20 overflow-hidden font-mono tracking-tight text-xs">
                <CustomLink href={`/${entity.slug}`} className="group/link flex items-center hover:text-orange-600 dark:hover:text-orange-400 transition-colors duration-300">
                  ...
                  <div className="min-w-0 flex-1 overflow-hidden">
                    <div className="text-xs text-foreground group-hover/link:text-orange-600 dark:group-hover/link:text-orange-400 transition-colors truncate">
                      <span className="max-w-[200px] truncate">{entity.name && entity.name}</span>{entity.type === "PUBLIC" && <span className="border bg-orange-50/90  dark:bg-orange-950/90 text-xs px-1 py-0.5 rounded-md ml-1">{entity.ticker}</span>}
                    </div>
                  </div>
                </CustomLink>
              </TableCell>
              ...
              <TableCell className="py-3 px-2 text-right w-[7%] font-mono tracking-tight text-xs">
                <div className="flex items-center justify-start gap-0.5">
                  ...
                  <span className="text-xs text-foreground px-2 font-mono tracking-tight">
                    {entity.bitcoinHoldings ? Math.round(Number(entity.bitcoinHoldings)).toString() : '—'}
                  </span>
                </div>
              </TableCell>
              ...
              <TableCell className="py-3 px-2 text-left w-[7%] font-mono tracking-tight text-xs">
                {entity.enterpriseValue ? (
                  <span className="text-xs text-foreground px-2 font-mono tracking-tight">
                    ${Math.round(Number(entity.enterpriseValue) / 1000000).toLocaleString()}M
                  </span>
                ) : (
                  <span className="text-muted-foreground text-xs">—</span>
                )}
              ...
              <TableCell className="py-3 px-2 text-left w-[7%] font-mono tracking-tight text-xs">
                <span className="text-xs text-foreground px-2 font-mono tracking-tight">
                  {entity.costBasis ? formatCostBasis(entity.costBasis) : '—'}
                </span>
              </TableCell>
