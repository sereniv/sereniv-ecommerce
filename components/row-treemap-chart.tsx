...
export default function RowTreemapChart({ treemapData, treemapLoading, loading, setTreemapData, forceHoldingsOnly }: TreemapChartProps) {
  ...
  if (forceHoldingsOnly) {
    return (
      <div className="relative overflow-hidden rounded-3xl border border-orange-200/50 dark:border-orange-800/50 bg-gradient-to-br from-background/90 to-background/70 backdrop-blur-sm shadow-2xl">
        <div className="w-full relative" style={{ height: "350px" }} ref={containerRef}>
  ...
  return (
    <>
      {header}
      {embedModal}
      <CardContent className="relative space-y-3 sm:space-y-4 md:space-y-6 p-0 sm:p-4 md:p-6">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 md:gap-4">
          <div className={cn(
            "flex items-center justify-center rounded-2xl bg-gradient-to-r from-orange-50/90 to-orange-100/70 dark:from-orange-950/50 dark:to-orange-900/30 backdrop-blur-sm p-1.5 sm:p-2 border border-orange-200/50 dark:border-orange-800/50 shadow-xl gap-1",
            isMobile ? "h-10 w-full" : isTablet ? "h-10" : "h-12"
          )}>
            {tabConfig.map((tab) => {
              const mobileLabels = {
              ...
              return (
                <button
                  key={tab.id}
                  className={cn(
                    "rounded-lg px-2 sm:px-4 py-1.5 sm:py-2 font-semibold transition-all duration-300 hover:scale-105 touch-manipulation flex-1 min-h-[32px] sm:min-h-[36px]",
                    isMobile ? "text-xs" : "text-sm",
                    activeTab === tab.id
                ...
                >
                  {isMobile ? mobileLabels[tab.label as keyof typeof mobileLabels] || tab.label : tab.label}
                </button>
              );
            })}
          </div>
